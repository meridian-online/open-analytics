...
`projection_fit_id` is stranger, and it is the sharper finding of the two: the same column, written by the same operator, from the same code, types as `unknown` in `medmcqa` and as `representation.text.word` in `supergpqa`. Both values are sixteen hex characters. The label is therefore a property of the sampled value rather than of what the column means, which is worth knowing before anyone builds on one.
